...
```tsx
// 👎 Avoid this
export default () => <form>...</form>// 👍 Name your functions
...
export default function Component() {
  ...
  return (
    <>
      <input
        value={value}onChange={e => setValue(e.target.value)}onBlur={validateInput}/>
      <button
        onClick={() => {
          if (isValid) {
            // ...
...
<UserProfilebio={user.bio}name={user.name}email={user.email}subscription={user.subscription}/>
...
function Component() {
  return (
    <CallToActionWidgetsubscribed={subscribed}registered={registered}/>)
}
```
  ...
  return (
    <>
      ...
      <Form>
        {({ values, setValue }) => (
          <input
            value={values.name}onChange={e => setValue('name', e.target.value)}/><input
